...
def get_ohlcv(sdate='20210501', edate='20210520', code=''):
...
#-------------------------------------------
#  KOSPI50 전일대비 등락율 가져오기
# ref : https://github.com/sharebook-kr/pykrx
#-------------------------------------------
def get_krx_kospi50(sdate, edate):
    ...
    for 종목분류코드 in 종목분류들:
        ...
        if 종목분류코드 == '1035': # KOSPI50 분류코드
            세부종목 = stock.get_index_portfolio_deposit_file(종목분류코드)
            print(세부종목)
            for ticker_code in 세부종목:
                indexdata = {}
                ticker_name = stock.get_market_ticker_name(ticker_code) # 종목명 가져오기
...
#---------------------------------------------------------
import pandas as pd
def naver_craw_시세종합(code='005930'):
...
def naver_craw_news(code):
    ...
    if response.status_code == 200:
        ...
        for td in trs:
            ...
            html_tab2 += "<td><a href='https://finance.naver.com"+td.select_one('a.tit').attrs['href'] +"'>"+td.select_one('a.tit').text+"</a></td>"
            html_tab2 += "</tr>"
        print(html_tab2)
    else:
        print(response.status_code)
